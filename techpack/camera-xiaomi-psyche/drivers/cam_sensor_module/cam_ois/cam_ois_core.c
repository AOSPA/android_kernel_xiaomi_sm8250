...
static int cam_bu24618_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (rc) {
		CAM_ERR(CAM_OIS, "Failed to locate .prog firmware %s", fw_name_prog);
		return rc;
	}
	...
	if (rc) {
		CAM_ERR(CAM_OIS, "Failed to locate .coeff firmware %s", fw_name_coeff);
		return rc;
	}
	...
	if (rc) {
		CAM_INFO(CAM_OIS, "no fw named .mem %s, skip", fw_name_mem);
		rc = 0;
	} else {
...
static int cam_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
    ...
    // Always apply default firmware update function for bu24618
